...
import appConfig, { LocalStaticUploadsConfig } from '../utils/appConfig.js';
import fs from 'fs';
 ...
 */
export default class LocalStaticUploadsDriver implements UploadsDriver {
  /**
   * Configuration for local uploads
   */
  private readonly config: LocalStaticUploadsConfig;
   ...
   */
  constructor(config: LocalStaticUploadsConfig) {
    this.config = config;
  }
  ...
  public createStorageEngine(): multer.StorageEngine {
    return multer.diskStorage({
      destination: (req, file, cb) => {
        const dir: string = this.config.static.path;
